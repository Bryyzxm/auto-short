...
// Import async job system and enhanced CORS
const {jobManager} = require('./services/asyncJobManager');
const {corsMiddleware, errorCorsMiddleware, timeoutMiddleware} = require('./services/enhancedCorsManager');
const {processVideoAsync} = require('./services/asyncVideoProcessor');
...
// Manual transcript upload feature removed: file upload handling is no longer supported.
// Previously this server used multer to accept .srt/.txt uploads for manual transcript synchronization.
// Kept intentionally as a comment for traceability; remove completely if you want zero footprint.
...
// Enhanced CORS and timeout middleware
...
//
// High-level segment processing used by the async video processor
//
async function processSegmentWithQualityAssurance(id, inputFile, outputFile, start, end, aspectRatio) {
 try {
  console.log(`[${id}] 🎯 Starting segment processing`);
  console.log(`[${id}] ▶️ Input: ${inputFile}`);
  console.log(`[${id}] 💾 Output: ${outputFile}`);
  console.log(`[${id}] ⏱️ Range: ${start}s → ${end}s, Aspect: ${aspectRatio}`);

  // Probe source to get dimensions
  const probeData = await new Promise((resolve, reject) => {
   execFile(FFPROBE_PATH, ['-v', 'quiet', '-print_format', 'json', '-show_streams', inputFile], (err, stdout, stderr) => {
    if (err) return reject(new Error(`ffprobe failed: ${err.message}`));
    try {
     resolve(JSON.parse(stdout || '{}'));
    } catch (e) {
     reject(new Error(`ffprobe output parse error: ${e.message}`));
    }
   });
  });

  const videoStream = Array.isArray(probeData?.streams) ? probeData.streams.find((s) => s.codec_type === 'video') : null;
  let videoWidth = Number(videoStream?.width) || 1280;
  let videoHeight = Number(videoStream?.height) || 720;
  const srcAspect = videoWidth && videoHeight ? videoWidth / videoHeight : 16 / 9;

  // Normalize aspect ratio value
  const aspect = ['9:16', '16:9', 'original'].includes(aspectRatio) ? aspectRatio : 'original';

  // Decide if upscaling is needed – target 720p height for processing
  const needsUpscaling = videoHeight < 720;

  console.log(`[${id}] 📐 Source resolution: ${videoWidth}x${videoHeight} (≈${srcAspect.toFixed(3)})`);
  console.log(`[${id}] 📏 Target aspect: ${aspect}, Upscale: ${needsUpscaling}`);
  ...
  if (!fs.existsSync(outputFile)) {
   throw new Error(`Output file not found after processing: ${outputFile}`);
  }

  // Log final probe (best effort)
  try {
   execFile(FFPROBE_PATH, ['-v', 'quiet', '-print_format', 'json', '-show_streams', outputFile], (probeErr, probeStdout) => {
    if (!probeErr && probeStdout) {
     const outProbe = JSON.parse(probeStdout);
     const outStream = outProbe.streams?.find((s) => s.codec_type === 'video');
     if (outStream) {
      console.log(`[${id}] ✅ Output resolution: ${outStream.width}x${outStream.height}`);
     }
    }
   });
  } catch (_) {}

  console.log(`[${id}] ✅ Segment processing completed: ${outputFile}`);
  return outputFile;
 } catch (error) {
  console.error(`[${id}] ❌ Segment processing failed: ${error.message}`);
  throw error;
 }
}
...
function cleanupOldVttFiles() {
 try {
  ...
  console.warn(`[CLEANUP] VTT cleanup warning: ${e.message}`);
 }
}
...
function cleanupOldMp4Files() {
 try {
  ...
  console.warn(`[CLEANUP] MP4 cleanup warning: ${e.message}`);
 }
}
...
app.post('/api/shorts', async (req, res) => {
 ...
 try {
  // Create async job for video processing
  const jobId = await jobManager.createJob('video_processing', {youtubeUrl, start, end, aspectRatio}, processVideoAsync);
...
app.get('/api/jobs/:jobId', (req, res) => {
 ...
 // Include result or error if job is completed
...
// Debug endpoint for job management
app.get('/api/jobs', (req, res) => {
 ...
 res.json({
  success: true,
  jobs: jobs,
  activeCount: jobs.filter((j) => j.status === 'processing' || j.status === 'queued').length,
  completedCount: jobs.filter((j) => j.status === 'completed').length,
  failedCount: jobs.filter((j) => j.status === 'failed').length,
 });
});

// Make functions globally available for async processor
global.azureEnv = azureEnv;
global.checkVideoFormats = checkVideoFormats;
global.buildYtDlpArgs = buildYtDlpArgs;
global.executeWithFallbackStrategies = executeWithFallbackStrategies;
global.YT_DLP_PATH = YT_DLP_PATH;
global.processSegmentWithQualityAssurance = processSegmentWithQualityAssurance;
...
// Manual transcript upload endpoint removed.
// The feature that accepted multipart transcript uploads and synchronized or generated segments
// has been intentionally removed to reduce surface area and maintenance overhead.
// If needed in the future, reintroduce an endpoint with strict validations and tests.
